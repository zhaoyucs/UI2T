            ...
            for step_i, batch in enumerate(self.train_loader):
                ...
                if self.verbose:
                    ...
                    if 'vsd' in self.args.tasks:
                        desc_str += f" VSD {task_counter['vsd']} | Loss {loss}"
...
def main_worker(gpu, args):
    ...
    if args.epochs > 0:
        ...
        if 'vsd' in args.tasks:
            print(f'Building CLS train loader at GPU {gpu}')
            vsd_train_loader = vsd_data.get_loader(
                vsd_args,
                split='train', mode='train', batch_size=vsd_args.batch_size,
                distributed=args.distributed, gpu=args.gpu,
                workers=args.num_workers,
                topk=args.train_topk,
            )
            train_loaders.append(vsd_train_loader)
    ...
    if gpu == 0:
        ...
        if args.epochs > 0:
            ...
            if 'vsd' in args.tasks:
                print(f'Building CLS val loader at GPU {gpu}')
                vsd_val_loader = vsd_data.get_loader(
                    cls_args,
                    split='val', mode='val', batch_size=cls_args.batch_size,
                    distributed=False, gpu=args.gpu,
                    workers=val_num_workers,
        ...
        if 'vsd' in args.tasks:
            print(f'Building VSD test loader at GPU {gpu}')
            vsd_test_loader = vsd_data.get_loader(
                vsd_args,
                split='test', mode='val', batch_size=caption_args.batch_size,
                distributed=False, gpu=args.gpu,
                workers=val_num_workers,
                topk=args.valid_topk,
            )
            test_loader['vsd'] = vsd_test_loader
