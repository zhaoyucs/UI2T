...
from continue_pretrain import tokenpermute_data,constituency_parsing_data
import vqa
import gqa
import nlvr
import vcr
...
class Trainer(TrainerBase):
    def __init__(self, args, train_loader=None, val_loader=None, test_loader=None, train=True):
        ...
        from continue_pretrain_model import VLT5MultiTask
        ...
        self.unfreeze_adapters() # unfreeze selected parameters
    ...
    def train(self):
        ...
        for epoch in range(self.args.epochs):
            ...
            if self.verbose:
                ...
                if 'vsd' in self.args.tasks:
                    # COCO Caption
                    caption_val_loader = self.val_loader['vsd']
                    valid_results = self.caption_evaluate(caption_val_loader)
                    valid_score = valid_results['CIDEr'] * 100
                    # valid_score = valid_results['Bleu_4'] * 100
                    if valid_score > best_caption_valid or epoch == 0:
                        best_caption_valid = valid_score
                        best_caption_epoch = epoch
                        #
        ...
        if self.verbose:
            ...
            if 'vsd' in self.args.tasks:
                # COCO Caption
                caption_test_loader = self.test_loader['vsd']
                test_results = self.caption_evaluate(caption_test_loader)
                for score_name, score in test_results.items():
                    wandb_log_dict[f'VSD/Test/{score_name}'] = score
                print(test_results)
...
def main_worker(gpu, args):
    ...
    vsd_args = deepcopy(args)
    vsd_args.batch_size = int(args.batch_size * 50 / 60)
    vsd_args.max_text_length = 40
    vsd_args.gen_max_length = 40
    ...
    if args.epochs > 0:
        if 'tp' in args.tasks:
            print(f'Building TP train loader at GPU {gpu}')
            tp_train_loader = tokenpermute_data.get_loader(
                vqa_args,
            ...
            train_loaders.append(tp_train_loader)
        
        if 'ctp' in args.tasks:
            print(f'Building CTP train loader at GPU {gpu}')
            ctp_train_loader = constituency_parsing_data.get_loader(
                vqa_args,
                split='karpathy_train', mode='train', batch_size=vqa_args.batch_size,
                distributed=args.distributed, gpu=args.gpu,
                workers=args.num_workers,
            ...
            train_loaders.append(ctp_train_loader)

            

    train_loader = continue_pretrain_data.MultitaskLoader(
        # [
        #     vqa_train_loader,
        #     gqa_train_loader,
        #     nlvr_train_loader,
    ...
    if gpu == 0:
        ...
        if args.epochs > 0:
            if 'vqa' in args.tasks:
                ...
                tp_val_loader = tokenpermute_data.get_loader(
                    vqa_args,
                    split='val', mode='val', batch_size=vqa_args.batch_size,
                    distributed=False, gpu=args.gpu,
                    workers=val_num_workers,
                    topk=args.valid_topk,
                )
                val_loader['tp'] = tp_val_loader
