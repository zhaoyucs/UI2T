...
class T5LayerFF(nn.Module):
    def __init__(self, config, has_adapter=False):
        super().__init__()
        ...
        if has_adapter and (config.use_adapter or config.use_compacter or config.use_lradapter):
            self.ff_adapter = AdapterController(config.adapter_config)
        else:
            self.ff_adapter = None


    def forward(self, hidden_states, block_adapters, adapter_weight):
        forwarded_states = self.layer_norm(hidden_states)
        forwarded_states = self.DenseReluDense(forwarded_states)
    
        if self.ff_adapter is not None:
            forwarded_states = self.ff_adapter(forwarded_states, adapter_weight)
...
class T5LayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, has_adapter=False):
        super().__init__()
    ...
    def forward(
        ...
        if self.attn_adapter is not None:
            y = self.attn_adapter(y, adapter_weight)
...
class T5LayerCrossAttention(nn.Module):
    def __init__(self, config, has_adapter=False):
        ...
        if (config.use_adapter or config.use_compacter or config.use_lradapter) and config.add_adapter_cross_attn and has_adapter:
...
class T5Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False, has_adapter=False):
        ...
        self.layer.append(T5LayerSelfAttention(config, has_relative_attention_bias=has_relative_attention_bias, has_adapter=has_adapter))
        if self.is_decoder:
            self.layer.append(T5LayerCrossAttention(config, has_adapter=has_adapter))

        self.layer.append(T5LayerFF(config, has_adapter=has_adapter))
    ...
    def forward(
        ...
        hidden_states = self.layer[-1](hidden_states, block_adapters, adapter_weight)
        if torch.isinf(hidden_states).any():
            clamp_value = torch.finfo(hidden_states.dtype).max - 1000
            hidden_states = torch.clamp(hidden_states, min=-clamp_value, max=clamp_value)
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, task_embed=None):
        ...
        self.block = nn.ModuleList(
            [T5Block(config, has_relative_attention_bias=bool(i == 0), has_adapter=((i>=config.num_layers/2) and not self.is_decoder)) for i in range(config.num_layers)]
        )
