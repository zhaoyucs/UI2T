...
class PTBDataset(Dataset):
    def __init__(self, split='train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train'):
        ...
        n_sent = 0
        data = []
        data_path = dataset_dir.joinpath('ptb/en_train_raw')
        target_path = dataset_dir.joinpath('ptb/train_02-21.LDC99T42')
        fdata = open(data_path,"r")
        ftarget = open(target_path,"r")
        for d,t in zip(fdata.readlines(),ftarget.readlines()):
            sent = d.strip()
            target = t.strip()
            new_datum = {
                'task': "ctp",
        ...
        if self.verbose:
            print(f"{self.source} has {n_sent} sentences")
    ...
    def __getitem__(self, idx):
        ...
        if self.args.no_prefix:
            ...
            prefix = "constituency tree parsing:"
            input_tokens = [prefix]

            ## disturb the tokens
            input_tokens.append(datum["sent"])
