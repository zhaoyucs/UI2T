...
class JointEncoder(T5Stack):
    def __init__(self, config, embed_tokens=None, task_embed=None):
        ...
        self.adapter_dispatcher = AdapterDispatcher(config)
    ...
    def forward(
        ...
        if self.config.num_layers > 0:
            ...
            for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
                ...
                if i == len(self.block) / 2:
                    adapter_weight, target_weight = self.adapter_dispatcher(task, hidden_states)
